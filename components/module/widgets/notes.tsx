    ...
    <BigWidgetLayout className={className}>
      ...
      >
        <div className="w-5/6 bg-white dark:bg-zinc-900 rounded-t-2xl sm:rounded-t-3xl flex flex-col h-[85%]">
          <header className="bg-gradient-to-t from-[#fabd00] to-[#fed202] dark:from-[#DAA500] dark:to-[#fabd00] rounded-t-2xl sm:rounded-t-3xl flex items-center text-white text-lg sm:text-xl gap-3 px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex-shrink-0">
            <FolderClosed
          ...
          <div
            className="flex flex-col gap-4 px-4 sm:px-6 lg:px-8 py-3 sm:py-4 relative flex-1 overflow-y-auto scrollbar-hide"
            role="list"
          ...
          >
            {NOTES.map((note, index) => (
              <article key={note.id} role="listitem" className="group">
                <div className="py-2">
                  <p className="text-sm sm:text-base leading-relaxed dark:text-zinc-300 mb-3">
                    {note.text}
                  </p>
                  <div
                ...
                {index < NOTES.length - 1 && (
                  <hr
                    className="border-zinc-200 dark:border-zinc-700 w-full mt-3"
                    aria-hidden="true"
                  />
                )}
              </article>
