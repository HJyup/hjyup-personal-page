...
function SongInfo({ title, artist }: SongInfoProps) {
  return (
    <div className="flex flex-col items-start text-left w-full min-w-0 flex-1">
      <h3 className="text-neutral-900 dark:text-neutral-100 text-base font-medium truncate w-full">
        {title}
      </h3>
...
function VolumeControl() {
  return (
    <div
      ...
      className="bg-neutral-200/50 dark:bg-[hsl(0,0%,15%)] h-10 w-fit px-2 rounded-full flex items-center shadow-sm cursor-pointer transition-all duration-200"
    >
      <div className="transition-all duration-200">
        <IoVolumeHigh className="w-6 h-6 text-neutral-500 dark:text-neutral-400" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex justify-between">
      <div className="flex justify-between w-full gap-2">
        <div className="bg-neutral-200/50 dark:bg-[hsl(0,0%,15%)] h-10 w-fit px-5 rounded-full flex items-center justify-center gap-4 shadow-sm">
          <IoPlayBack className="w-6 h-6 text-neutral-500 dark:text-neutral-400" />
          <IoPlay className="w-6 h-6 text-neutral-500 dark:text-neutral-400" />
          <IoPlayForward className="w-6 h-6 text-neutral-500 dark:text-neutral-400" />
        </div>
...
export const MusicWidget = forwardRef<HTMLDivElement, MusicWidgetProps>(
  (
    ...
    return (
      ...
      >
        <IoMusicalNotes className="absolute top-4 right-4 md:top-5 md:right-5 lg:top-6 lg:right-6 w-5 h-5 text-neutral-500 dark:text-neutral-400" />
